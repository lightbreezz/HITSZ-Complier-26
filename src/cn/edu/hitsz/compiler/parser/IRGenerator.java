...
public class IRGenerator implements ActionObserver {
        ...
        switch (production.index()) {
            case 6 -> {
                ...
                if (!symbolTable.has(name)) {
                    throw new RuntimeException("No such id in symbol table: %s".formatted(name));
                }

                final var target = IRVariable.named(name);
            ...
            case 7 -> {
                // S -> return E
                final var returnValue = irStack.pop().irValue();
                irStack.pop();
    ...
    private final Deque<IRSymbol> irStack = new ArrayDeque<>();
    private static class IRSymbol {
        ...
        Token token() {
            if (token == null) {
                throw new RuntimeException("IR symbol does not hold token");
            }
            return token;
        }
        ...
        IRValue irValue() {
            if (value == null) {
                throw new RuntimeException("IR symbol does not hold IR value");
            }
            return value;
        }
