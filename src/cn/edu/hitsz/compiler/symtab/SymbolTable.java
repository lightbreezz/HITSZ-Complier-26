...
public class SymbolTable {
    ...
    public SymbolTableEntry get(String text) {
        ...
        if (entry == null) {
            throw new RuntimeException(String.format("Symbol '%s' does not exist", text));
        }
        return entry;
    }
    ...
    public SymbolTableEntry add(String text) {
        if (has(text)) {
            throw new RuntimeException(String.format("Symbol '%s' already exists", text));
        }
        // 跟C语言的auto一样
    ...
    // 定义一个哈希表就好
    private Map<String, SymbolTableEntry> entries = new HashMap<>();
}
